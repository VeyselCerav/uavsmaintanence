...
import { createFMEA, updateFMEA } from "@/services/fmea";
import { listComponentTypes } from "@/services/maintenance";
import { listClasses, listMissions, listPlatforms } from "@/services/uavs";
import type { FMEA, FMEAWritePayload } from "@/types/fmea";

type Props = {
  initial?: FMEA;
};

export function FMEAHeaderForm({ initial }: Props) {
  const { t } = useI18n();
  ...
  const [form, setForm] = useState<FMEAWritePayload>({
    code: initial?.code ?? "",
    title: initial?.title ?? "",
  ...
  function setField<K extends keyof FMEAWritePayload>(key: K, value: FMEAWritePayload[K]) {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      ...
      const saved = initial ? await updateFMEA(initial.id, payload) : await createFMEA(payload);
      router.replace(`/fmea/${saved.id}`);
    } catch (err) {
    ...
    <form onSubmit={onSubmit} className="max-w-3xl space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label={t("uav.mission")}>
          ...
          >
            <option value="">{t("fmea.allMissions")}</option>
      ...
      <div className="flex gap-2">
        ...
        <Button asChild variant="outline">
          <Link href={initial ? `/fmea/${initial.id}` : "/fmea"}>{t("common.cancel")}</Link>
        </Button>
      </div>
    </form>
  );
